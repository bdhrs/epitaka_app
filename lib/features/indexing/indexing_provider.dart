...
final ftsAvailableVersionsProvider = FutureProvider<List<TranslationVersion>>((
  ref,
) async {
...
final ftsDownloadableVersionsProvider =
    FutureProvider<List<TranslationVersion>>((ref) async {
      final merged = await ref.watch(mergedTranslationVersionsProvider.future);
      return merged.where((v) => !v.isAvailable && v.hasDownloadUrl).toList()
        ..sort((a, b) {
          if (a.languageCode == 'en') return -1;
          if (b.languageCode == 'en') return 1;
          return a.englishName.compareTo(b.englishName);
...
///
/// A 0-byte stray file (left by an interrupted copy or an empty bundled
/// asset) must NOT count as installed — otherwise the setup wizard would
/// either skip a database the app can't open, or keep asking for a download
/// that is already on disk, depending on which check runs first.
...
final ftsRequiredCoreAssetsProvider = FutureProvider<List<CoreAsset>>((
  ref,
) async {
...
final ftsAllRequiredReadyProvider = FutureProvider<bool>((ref) async {
  ...
  final transRequired = await ref.watch(ftsRequiredTranslationsProvider.future);
