...
# Normalise modes to 755/644. Without this the build machine's umask travels
# into the package: cp -a preserves the bundle's modes and a umask of 002 (the
# default on Ubuntu/Mint for the user's own group) yields group-writable dirs
# and files owned root:root, which Lintian reports as
# non-standard-file-perm/-dir-perm. Executables keep their bit — the launcher
# and the bundled .so files must stay executable, so the u+x test drives which
# of the two modes a file gets. -type d/f both exclude the /usr/bin symlink, so
# it keeps its own mode and its target is never chmod'd through it.
...
# -Zxz because the payload is dominated by SQLite databases, which compress
# very well: measured on ubuntu-latest, an ~812MB payload became a ~150MB
# package in under two minutes, a small addition to the Linux job's runtime.
